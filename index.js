...
var ASM_CONSTS = {
  70072: () => { FS.syncfs(false, function (err) { if (err) console.error('Error syncing to IndexedDB', err); }); },  
 70169: () => { if (!FS.analyzePath('/data').exists) { FS.mkdir('/data'); } FS.mount(IDBFS, {}, '/data'); FS.syncfs(true, function (err) { if (err) console.error("Error loading IDBFS:", err); }); }
};
